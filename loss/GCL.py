...
class GCL(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        #
        # return loss
class GCL2(nn.Module):
    def __init__(self, num_pos=4, feat_norm='no', threshold=1e-6):
        super(GCL2, self).__init__()
        self.num_pos = num_pos
        self.feat_norm = feat_norm
    ...
    def forward(self, inputs, targets,method='eucilidean'):
        ...
        is_neg = targets.expand(N, N).ne(targets.expand(N, N).t())  # is_neg:96,96
        is_g_neg = is_neg[num:num * 3, :]

        _, input_gc,input_gt, _ = inputs.chunk(4, 0)
        input_g=torch.concat([input_gc,input_gt],dim=0)
        ...
        mask_an = (an - d_neg).expand(num, 2*N - 8 * self.num_pos).lt(0)  # mask
        an = an * mask_an
