...
class GMSEL(nn.Module):
    ...
    def forward(self, inputs, targets,method='eucilidean'):
        ...
        # return loss
class GMSEL2(nn.Module):
    def __init__(self,num_pos,feat_norm = 'no'):
        super(GMSEL2, self).__init__()
        self.num_pos = num_pos
        self.feat_norm = feat_norm
    ...
    def forward(self, inputs, targets,method='eucilidean'):
        ...
        input_list=inputs.chunk(4,0)
        target_list=targets.chunk(4,0)
        N=target_list[0].size(0)
        is_pos = target_list[0].expand(N, N).eq(target_list[0].expand(N, N).t())
        is_pos = torch.cat([is_pos,is_pos],dim=0)

        if method=='eucilidean':
            dist_g_r = pdist_torch(torch.cat(input_list[1:3]), input_list[0])
        elif method=='cosine':
            dist_g_r1 = pdist_cosine(input_list[1], input_list[0]).cuda()
            dist_g_r2 = pdist_cosine(input_list[2], input_list[0]).cuda()
            dist_g_r=torch.cat([dist_g_r1,dist_g_r2],dim=0)
        dist_g_r = dist_g_r[is_pos].contiguous().view(2*N, -1)  # [N, num_pos]
        dist_g_r=torch.mean(dist_g_r,dim=1)

        if method=='eucilidean':
            dist_g_i = pdist_torch(torch.cat(input_list[1:3]), input_list[-1])
        elif method=='cosine':
            dist_g_i1 = pdist_cosine(input_list[1], input_list[2]).cuda()
            dist_g_i2 = pdist_cosine(input_list[2], input_list[2]).cuda()
            dist_g_i=torch.cat([dist_g_i1,dist_g_i2],dim=0)
        dist_g_i = dist_g_i[is_pos].contiguous().view(2*N, -1)  # [N, num_pos]
        dist_g_i= torch.mean(dist_g_i, dim=1)


        loss = torch.mean(torch.pow(dist_g_r-dist_g_i,2))
        # loss = torch.mean(torch.pow(dist_g_r - dist_g_i, 2)) #/ 2
